...
declare variable $local:wants-json := tokenize(request:get-header("Accept"), ", ?") =
    "application/json";

declare variable $local:is-ajax-request := request:get-header("X-Requested-With") =
    "XMLHttpRequest";
...
declare function local:user-allowed ($user as xs:string?) as xs:boolean {
    $local:config/restrictions/@guest = "yes" or
        (not(empty($user)) and not($user = ("guest", "nobody")))
};

declare function local:query-execution-allowed (
    $user as xs:string?,
    $is-dba as xs:boolean
) as xs:boolean {
    $is-dba or ($local:config/restrictions/@execute-query = "yes" and local:user-allowed($user))
};
...
if ($exist:path eq "") then
    ...
    let $resource := if ($user-allowed) then
        "index.html"
    else
        "login.html"
...
else if (not($user-allowed)) then (
    if ($local:wants-json or $local:is-ajax-request) then (
        ...
        <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
            <redirect method="get" url="login.html" />
            <!-- maybe add additional parameters -->
        </dispatch>
)
...
(:
 ...
 :)
    ...
    util:declare-option("exist:serialize", "method=json media-type=application/json"),
    response:set-header("Content-Type", "application/json; charset=UTF-8"),
    <status><user>{ $user }</user><isAdmin json:literal="true">{ $user-is-dba }</isAdmin></status>
) else if ($local:method = ("get", "post") and $exist:resource = ("login.html", "login")) then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    ...
    return if (not($xquery-execution-allowed)) then
...
(: Block abandoned/non-functional modules :)
    ...
    response:set-status-code(410), <gone>{ $exist:resource } is no longer available.</gone>
)
