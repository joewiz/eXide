...
declare function editor:completions ($request as map(*)) {
    ...
    let $sources := if ($sources instance of xs:string) then
        $sources
    else if (exists($sources)) then
        $sources?*
    else (
    )
    let $uris := if ($uris instance of xs:string) then
        $uris
    else if (exists($uris)) then
        $uris?*
    else (
    )
    let $mprefixes := if ($mprefixes instance of xs:string) then
        $mprefixes
    else if (exists($mprefixes)) then
        $mprefixes?*
    else (
    )
    ...
    return array
        {
            if ($q) then
                editor:get-built-in-functions($q)
            else (
            ),
            if ($signature) then
                let $desc := editor:get-by-signature($signature)
                return if (exists($desc)) then
                    $desc
                else
                    editor:get-imported-functions(
                        $q,
                        $signature,
                        $base,
                        $sources,
...
declare function editor:symbols ($request as map(*)) {
    ...
    let $prefixes := if ($prefixes instance of xs:string) then
        $prefixes
    else if (exists($prefixes)) then
        $prefixes?*
    else (
    )
    let $bases := if ($bases instance of xs:string) then
        $bases
    else if (exists($bases)) then
        $bases?*
    else (
    )
    ...
    return array
        {
            ...
            let $base := $bases[$i]
            let $module-source := if (matches($source, "^(/|\w+:)")) then
                $source
                        ...
                        array
                            {
                                ...
                                let $name := if (contains($fn/@name, ":")) then
                                    substring-after($fn/@name, ":")
                                else
                                    string($fn/@name)
                                return map {
                                    "name": $prefix || ":" || $name,
                                    "signature":
                                        editor:generate-signature($fn, $prefix, $name, false()),
                                    "visibility":
                                        if ($fn/annotation/@name = "private") then
                                            "private"
                                        else
                                            "public"
                                }
    ...
    return array
        {
            ...
            where exists($module) and (empty($prefix) or starts-with($module/@prefix, $prefix))
...
declare function editor:validate ($request as map(*)) {
    ...
    return try {
        let $parsed := if ($validate) then
            validation:jaxp-parse($xml, true(), doc("/db/apps/eXide/resources/schema/catalog.xml"))
        else
            parse-xml($xml)
        ...
        map {
            "status": "invalid",
            "errors": array { map {"line": $err:line-number, "message": $err:description} }
        }
    }
};
...
declare function editor:imports ($request as map(*)) {
    ...
    let $imported := if ($imported instance of xs:string) then
        $imported
    else if (exists($imported)) then
        $imported?*
    else (
    )

    let $path := if (starts-with($path, "xmldb:exist://")) then
        substring-after($path, "xmldb:exist://")
    else
        $path

    (: Find the package root for this file :)
    let $pkg-root := editor:get-package-root($path)
    let $scan-root := if (exists($pkg-root)) then
        $pkg-root
    else
        $path
    ...
    let $local-modules := editor:sort-modules(
        dbutil:find-by-mimetype(
            ...
            function ($script) {
                ...
                return if ($info?namespace = $imported) then (
    ...
    let $global-modules := editor:sort-modules(
        ...
        return try {
            ...
            return if (empty($prefix) or $module/@prefix = $prefix) then
                map {
...
declare %private function editor:get-by-signature ($signature as xs:string) {
    ...
    return if (exists($fn)) then
        ...
        let $function-name := if (contains($desc/@name, ":")) then
...
declare %private function editor:get-built-in-functions ($q as xs:string) {
    let $supplied-prefix := if (contains($q, ":")) then
        substring-before($q, ":")
    else (
    )
    let $name-fragment := if (contains($q, ":")) then
        substring-after($q, ":")
    else
        $q
    let $show-fn := exists($supplied-prefix) and $supplied-prefix eq "fn"
    let $modules := if ($supplied-prefix eq "fn") then
        ...
        return if ($supplied-prefix) then
            $all[starts-with(@prefix, $supplied-prefix)]
        else
            $all
    let $functions := $modules/function[not(annotation/@name = "private") and not(deprecated)]
    for $function in $functions
    let $function-name := if (contains($function/@name, ":")) then
        substring-after($function/@name, ":")
    else
        $function/@name
    let $module-prefix := $function/parent::module/@prefix
    let $complete-name := if ($show-fn) then (
        "fn:" || $function-name
    ) else (
        $module-prefix || ":" || $function-name
    )
...
declare %private function editor:get-imported-functions (
    ...
    let $supplied-prefix := if (empty($signature)) then
        replace($q, "^\$?([^:]+):?.*$", "$1")
    else
        replace($signature, "^\$?([^:]+):?.*$", "$1")
    for $prefix at $i in $prefixes
    where matches($prefix, "^" || $supplied-prefix)
    let $source-url := if (matches($sources[$i], "^(/|\w+:)")) then
        $sources[$i]
    else if (exists($base)) then
        concat($base, "/", $sources[$i])
    else
        $sources[$i]
    return try {
        ...
        return (
            if (not(starts-with($q, "$")) and not(starts-with($signature, "$"))) then
                ...
                for $fn in $module/function[not(annotation/@name = "private")]
                let $fn-name := if (contains($fn/@name, ":")) then
                    substring-after($fn/@name, ":")
                else
                    string($fn/@name)
                ...
                return if (
                    (empty($signature) or $signature = $complete-name || "#" || $arity) and
                    ...
                    map {
                        ...
                        "leftLabel":
                            string($fn/returns/@type) ||
                                editor:cardinality($fn/returns/@cardinality),
...
declare %private function editor:describe-function ($function, $prefix, $name, $show-fn, $q) {
    map {
        ...
        "leftLabel":
            string($function/returns/@type) || editor:cardinality($function/returns/@cardinality),
        "replacementPrefix": $q
    }
};
...
declare %private function editor:generate-signature ($function, $prefix, $name, $show-fn) {
    (
        ...
        string-join(
            $function/argument!(
                "$" || ./@var || " as " || ./@type || editor:cardinality(./@cardinality)
            ),
            ", "
        ) ||
        ")"
...
declare %private function editor:get-package-root ($path as xs:string) as xs:string? {
    (
        for $pkg in
            collection(repo:get-root())/expath:package[starts-with($path, util:collection-name(.))]
...
declare %private function editor:get-module-info ($script as xs:anyURI) {
    ...
    return let $match := analyze-string(
            $source,
            "^module\s+namespace\s+([^\s=]+)\s*=\s*['""]([^'""]+)['""]",
            "m"
        )//fn:match
        return map {
