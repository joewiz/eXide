...
declare variable $db:copy-collection := let $fnNew := function-lookup(
    xs:QName("xmldb:copy-collection"),
    2
)
return if (exists($fnNew)) then
    $fnNew
else
    function-lookup(xs:QName("xmldb:copy"), 2);

declare variable $db:copy-resource := let $fnNew := function-lookup(
    xs:QName("xmldb:copy-resource"),
    4
)
return if (exists($fnNew)) then
    $fnNew
...
declare function db:get ($request as map(*)) {
    ...
    return if (xmldb:collection-available($path)) then
        ...
        roaster:response(404, "application/json", map {"error": "Not found: " || $path})
};
...
declare function db:put ($request as map(*)) {
    ...
    return try {
        ...
        let $_ := if ($mime = "application/xquery") then
            sm:chmod($stored, "rwxr-xr-x")
        else (
        )
        return map {"status": "ok", "path": $stored}
    ...
    return switch ($action)
        ...
        case "copy" return
            try {
                ...
                let $_ :=
                    ...
                    return if (xmldb:collection-available($enc-source)) then
        ...
        case "rename" return
            try {
                ...
                return if (xmldb:collection-available($path)) then (
                    xmldb:rename($path, $target), map {"status": "ok"}
                ) else
                    let $col := replace($path, "/[^/]+$", "")
                    let $name := replace($path, "^.*/", "")
                    return (xmldb:rename($col, $name, $target), map {"status": "ok"})
            } catch * { roaster:response(400, "application/json", map {"error": $err:description}) }
        default return
            roaster:response(400, "application/json", map {"error": "Unknown action: " || $action})
};
...
declare function db:delete ($request as map(*)) {
    ...
    return try {
        if (xmldb:collection-available($path)) then (
            xmldb:remove($path), map {"status": "ok"}
        ) else
            let $col := replace($path, "/[^/]+$", "")
            let $name := replace($path, "^.*/", "")
            return (xmldb:remove($col, $name), map {"status": "ok"})
    } catch * { roaster:response(400, "application/json", map {"error": $err:description}) }
};
...
declare function db:patch ($request as map(*)) {
    ...
    let $body := $request?body
    return try {
        if (exists($body?owner)) then
            sm:chown($path, $body?owner)
        else (
        ),
        if (exists($body?group)) then
            sm:chgrp($path, $body?group)
        else (
        ),
        if (exists($body?mode)) then
            sm:chmod($path, $body?mode)
        else (
        ),
        if (exists($body?mime)) then
            xmldb:set-mime-type($path, $body?mime)
        else (
        ),
        map {"status": "ok"}
    } catch * { roaster:response(403, "application/json", map {"error": $err:description}) }
};
...
declare %private function db:encode-path ($path as xs:string) as xs:string {
    string-join(
        ...
        return if ($segment = "") then
            ""
        else
            xmldb:encode($segment),
        "/"
    )
};
...
declare %private function db:browse-collection ($path, $request, $user) {
    ...
    let $all-items := (
        ...
        return let $child-path := $path || "/" || $child
...
declare %private function db:load-document ($path, $request, $user) {
    ...
    return if (not(sm:has-access(xs:anyURI($path), "r"))) then
        roaster:response(404, "application/json", map {"error": "Not found or no read access"})
        ...
        if ($download) then
            ...
            let $is-text := matches(
                $mime,
                "^(text/|application/(xquery|javascript|json|xml|xslt\+xml|xsl-fo|css|less))"
            )
            let $content := if ($is-text) then
                try { util:binary-to-string(util:binary-doc($path)) } catch * { () }
            else (
            )
            return map:merge(
                (
                    map {
                        ...
                        "size":
                            xmldb:size(replace($path, "/[^/]+$", ""), replace($path, "^.*/", "")),
                        "lastModified":
                            string(
                                xmldb:last-modified(
                                    replace($path, "/[^/]+$", ""),
        ...
        let $serialization-opts := "expand-xincludes=" ||
            (
                if ($expand-xincludes) then
                    "yes"
                else
                    "no"
            )
