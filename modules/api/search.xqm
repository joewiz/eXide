...
declare function search:search ($request as map(*)) {
    ...
    let $flags := if ($case-sensitive) then
        ""
    else
        "i"
    let $search-str := if ($use-regex) then
        $query-str
    else
        search:escape-regex($query-str)

    return if (empty($search-str) or $search-str = "") then
        roaster:response(400, "application/json", map {"error": "Empty search query"})
    else
        let $hits := if ($type = "all") then
            dbutil:scan(
                xs:anyURI($collection),
                function ($col, $resource) {
        ...
        return map {"query": $query-str, "collection": $collection, "hits": array { $hits }}
};
...
declare %private function search:search-resource ($resource, $search-str, $flags) {
    ...
    return if ($analyzed/fn:match) then
