...
declare function templates:list ($request as map(*)) {
    ...
    return if (doc-available($doc-path)) then
        ...
        return map:merge(
            ...
            return map:entry(
                ...
                array
                    {
                        for $t in $template
                        return map {"name": string($t/@name), "description": string($t/description)}
                    }
            )
        )
    else
...
declare function templates:get ($request as map(*)) {
    ...
    let $doc-path := $config:app-root || "/templates/documents.xml"
    let $code := if (doc-available($doc-path)) then
    ...
    return if ($code) then
        ...
        roaster:response(404, "application/json", map {"error": "Template not found: " || $name})
};
