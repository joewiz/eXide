...
declare variable $packages:copy-resource := let $fnNew := function-lookup(
...
declare variable $packages:ANT_FILE := <project default="xar" name="$$app$$">
...
declare function packages:list ($request as map(*)) {
    ...
    return map {"packages": array { $apps }}
};
...
declare function packages:install ($request as map(*)) {
    ...
    return if (exists($body?abbrev)) then
...
declare function packages:get ($request as map(*)) {
    ...
    let $resolved-abbrev := if (exists($collection) and $collection != "") then
        packages:abbrev-for-collection($collection)
    else
        $abbrev
    return if (empty($resolved-abbrev) or $resolved-abbrev = "") then
        roaster:response(
            404,
            "application/json",
        ...
        let $expath := doc($col || "/expath-pkg.xml")/expath:package
        let $repo-meta := doc($col || "/repo.xml")/repo:meta
        return if (exists($expath)) then
            let $user := sm:id()//sm:real/sm:username/string()
            let $is-admin := if ($user) then
                sm:is-dba($user)
            else
                false()
...
declare function packages:uninstall ($request as map(*)) {
    ...
    let $expath := doc($col || "/expath-pkg.xml")/expath:package
    return if (empty($expath)) then
        roaster:response(404, "application/json", map {"error": "Package not found: " || $abbrev})
    else
        try {
...
declare function packages:deploy ($request as map(*)) {
    ...
    return if (empty($expath)) then
        ...
        try {
            ...
            let $_ := (repo:remove($expath/@name), repo:install-and-deploy-from-db($pkg))
            return map {"status": "ok", "path": $col}
        } catch * { roaster:response(400, "application/json", map {"error": $err:description}) }
};
...
declare function packages:config ($request as map(*)) {
    ...
    return try {
        ...
        return map {"status": "ok", "collection": $collection}
    } catch * { roaster:response(400, "application/json", map {"error": $err:description}) }
};
...
declare %private function packages:install-xar ($request as map(*)) {
    ...
    return try {
        ...
        let $_ := (repo:remove($app-name), repo:install-and-deploy-from-db($pkg))
        return roaster:response(
            201,
            "application/json",
            map {
...
declare %private function packages:get-xar-descriptors ($zipPath) {
    ...
    return if (exists($binary)) then
        ...
        let $entryCb := function ($path as xs:anyURI, $type as xs:string, $param as item()*) {
            $path = "expath-pkg.xml"
        }
...
declare %private function packages:create-app ($body as map(*)) {
    ...
    return try {
        ...
        (: Store repo.xml :)
        let $repo-meta := <meta xmlns="http://exist-db.org/xquery/repo">
        ...
        let $_ := if (xmldb:collection-available($template-col)) then
            packages:copy-templates($collection, $template-col, ($user, $group), $permissions)
        else (
        )

        (: Store build.xml :)
        let $params := <parameters><param name="app" value="{ $abbrev }" /></parameters>
        let $_ := xmldb:store(
            $collection,
            "build.xml",
        ...
        return roaster:response(201, "application/json", map {"status": "ok", "path": $collection})
    } catch * { roaster:response(400, "application/json", map {"error": $err:description}) }
};

declare %private function packages:mkcol (
    $path,
    $userData as xs:string*,
    $permissions as xs:string?
) {
    let $path := if (starts-with($path, "/db/")) then
...
declare %private function packages:mkcol-recursive (
    $collection,
    $components,
    $userData as xs:string*,
    $permissions as xs:string?
    ...
    if (exists($components)) then
        let $perms := if ($permissions) then
            packages:set-execute-bit($permissions)
        else
            "rwxr-x---"
...
declare %private function packages:copy-templates (
    $target as xs:string,
    $source as xs:string,
    $userData as xs:string+,
    $permissions as xs:string
    ...
    return if (xmldb:collection-available($source)) then (
        ...
        return (
            ...
            let $perms := if ($mime eq "application/xquery") then
                packages:set-execute-bit($permissions)
            else
                $permissions
            return (
