...
declare function wsmon:push-status ($request as map(*)) {
    ...
    let $running-queries := try { system:get-running-xqueries() } catch * { () }
    ...
    let $jmx-data := if ($jmx-token) then
        try {
            let $url := request:get-scheme() ||
                "://localhost:" ||
                request:get-server-port() ||
                request:get-context-path() ||
                "/status?c=instances&amp;c=processes&amp;c=memory&amp;token=" ||
                $jmx-token
    ...
    let $recent := if ($jmx-data) then
        ...
        return map {
            ...
            "mostRecentExecutionDuration":
                string(($v/*[local-name() = "mostRecentExecutionDuration"])[1]),
    ...
    let $active-brokers := if ($jmx-data) then
        string(($jmx-data//*[local-name() = "ActiveBrokers"])[1])
    else
        "--"
    let $max-brokers := if ($jmx-data) then
    ...
    let $data := map {
        ...
        "db": map {"activeBrokers": $active-brokers, "maxBrokers": $max-brokers},
        "runningQueries":
            if ($jmx-data) then
                count(
                    $jmx-data//*[local-name() = "RunningQueries"]/*[local-name() =
                        "row"]/*[local-name() = "value"][not(*[local-name() = "caller"] = "true")]
                )
            else
                count($running-queries//system:query[not(@caller = "true")]),
        "queries":
            array
                {
                    if ($jmx-data) then
                        for $row in
                            $jmx-data//*[local-name() = "RunningQueries"]/*[local-name() = "row"]
                        let $v := $row/*[local-name() = "value"]
                        let $src := string(($v/*[local-name() = "sourceKey"])[1])
                        where not($v/*[local-name() = "caller"] = "true") and
                            not(contains($src, "/apps/eXide/modules/api/"))
                        return map {
...
declare %private function wsmon:get-jmx-token () as xs:string? {
    try {
        ...
        return if (exists($file-read) and exists($file-exists)) then
