...
declare function sync:sync ($request as map(*)) {
    ...
    let $after := if (exists($body?after) and $body?after ne "") then
    ...
    return if (empty($dir) or $dir = "") then
        roaster:response(
            400,
            "application/json",
        ...
        roaster:response(
            ...
            map {
                "error":
                    "Filesystem sync requires eXist-db's file:sync() function, which is not available on this server."
            }
        )
    else
        try {
...
declare %private function sync:working-dir-from-descriptor (
    $collection as xs:string
) as xs:string? {
    let $root := sync:get-app-root($collection)
    return if (exists($root)) then
