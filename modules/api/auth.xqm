...
declare function auth:is-allowed ($user as xs:string?) as xs:boolean {
    ...
    return $conf/restrictions/@guest = "yes" or (exists($user) and not($user = ("guest", "nobody")))
};
...
declare function auth:login ($request as map(*)) {
    ...
    return if (auth:is-allowed($user)) then
        map {"user": ($user, "guest")[1], "isAdmin": sm:is-dba(($user, "guest")[1])}
    else
        roaster:response(401, "application/json", map {"error": "unauthorized"})
};
