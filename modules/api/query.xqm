...
declare function query:execute ($request as map(*)) {
    ...
    return if (not($allowed)) then
        roaster:response(403, "application/json", map {"error": "Query execution not allowed"})
    else
        try {
...
declare function query:compile ($request as map(*)) {
    ...
    let $_ := if (exists($ws-send)) then
        $ws-send(
            "eXide",
            map {"type": "textDocument/publishDiagnostics", "uri": $uri, "diagnostics": $errors}
        )
    else (
    )
    return map {"errors": $errors}
};
...
declare function query:completions ($request as map(*)) {
    ...
    let $ns-prefix := if (contains($prefix, ":")) then
        substring-before($prefix, ":")
    else
        $prefix
    let $inspect-results := if ($ns-prefix = "" or exists($lang-results)) then (
    ) else
        (: Extract the namespace URI for this prefix from the query's import statements :)
        let $import-pattern := "import\s+module\s+namespace\s+" ||
            $ns-prefix ||
            "\s*=\s*[""']([^""']+)[""']"
        let $match := analyze-string($xquery, $import-pattern)//fn:match/fn:group[1]/string()
        let $ns-uri := if (exists($match)) then
            $match[1]
        (: Also check registered/mapped modules by prefix :)
        else
            query:uri-for-prefix($ns-prefix)
        return if (empty($ns-uri)) then (
        ) else
            try {
                let $module := inspect:inspect-module-uri(xs:anyURI($ns-uri))
                for $func in
                    $module//function[(@visibility = "public" or not(@visibility)) and
                        (argument or @arity = "0")]
...
declare %private function query:build-signature ($func as element()) as xs:string {
    ...
    let $params :=
        ...
        return "$" ||
            ...
            (
                if ($arg/@type) then
                    " as " || $arg/@type/string() || query:cardinality-symbol($arg/@cardinality)
                else
                    ""
            )
    ...
    return $name ||
        ...
        (
            if ($return/@type) then
                " as " || $return/@type/string() || query:cardinality-symbol($return/@cardinality)
            else
                ""
        )
};
...
declare %private function query:make-snippet ($detail as xs:string) as xs:string {
    ...
    let $params := analyze-string(
        $detail,
        "\$([a-zA-Z][a-zA-Z0-9\-_\.]*)"
    )//fn:match/fn:group[1]/string()
    return if (empty($params)) then
        $fname || "()"
    else
        $fname ||
            "(" ||
...
declare function query:results ($request as map(*)) {
    ...
    let $ser-params := map:merge(
        (
            ...
            if (exists($request?parameters?("omit-xml-declaration"))) then
                map {"omit-xml-declaration": $request?parameters?("omit-xml-declaration")}
            else (
            ),
            if (exists($request?parameters?encoding)) then
                map {"encoding": $request?parameters?encoding}
            else (
            ),
            if (exists($request?parameters?("media-type"))) then
                map {"media-type": $request?parameters?("media-type")}
            else (
            ),
            if (exists($request?parameters?("item-separator"))) then
                map {"item-separator": $request?parameters?("item-separator")}
            else (
            ),
            (: eXist-specific: highlight index matches :)
            if (exists($request?parameters?("highlight-matches"))) then
                map {"highlight-matches": $request?parameters?("highlight-matches")}
            else (
            )
        )
    )
    ...
    return try {
        let $page := if (exists($fetch4)) then
            $fetch4($id, $start, $count, $ser-params)
        else
            cursor:fetch($id, $start, $count)
        return map {"start": $start, "count": array:size($page), "items": $page}
    } catch * {
        roaster:response(
            404,
            "application/json",
...
declare function query:close ($request as map(*)) {
    let $id := $request?parameters?id
    return map {"closed": cursor:close($id)}
};
