...
import module namespace xqsuite = "http://exist-db.org/xquery/xqsuite" at "resource:org/exist/xquery/lib/xqsuite/xqsuite.xql";
...
declare function test:run ($request as map(*)) {
    ...
    return if (not(util:binary-doc-available($db-uri))) then
        roaster:response(
            400,
            "application/json",
            map {
                "error":
                    "Module not found or not saved in database. Save the test module to a database collection first.",
                "source": $source
            }
        )
    else
        try {
            ...
            roaster:response(
                400,
                "application/json",
                map {"error": $err:description, "code": string($err:code), "source": $source}
            )
        }
};
