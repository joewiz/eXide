...
declare function git:process (
    $command as xs:string,
    $option as xs:string?,
    $workingDir as xs:string
) {
    let $process-option := <option><workingDir>{ $workingDir }</workingDir></option>
...
let $collection := if ($collectionParam) then
