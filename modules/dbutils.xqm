...
declare function dbutil:scan-collections (
    ...
    if (sm:has-access($root, "rx")) then
        ...
        return dbutil:scan-collections(xs:anyURI($root || "/" || $child), $func)
    else (
    )
};
...
(:~
 ...
 :)
declare function dbutil:scan-resources (
    $collection as xs:anyURI,
    $func as function (xs:anyURI) as item()*
) {
    if (sm:has-access($collection, "rx")) then
        for $child in xmldb:get-child-resources($collection)
        return $func(xs:anyURI($collection || "/" || $child))
    else (
    )
};
...
declare function dbutil:scan (
    ...
    dbutil:scan-collections(
        $root,
        function ($collection as xs:anyURI) {
            $func($collection, ()),
