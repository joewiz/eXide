...
declare function local:retrieve ($num as xs:integer) as element() {
    ...
    let $auto-expand-matches := request:get-parameter(
        "auto-expand-matches",
        true()
    ) cast as xs:boolean
    let $cached-items := session:get-attribute("cached")
    let $cached-item := $cached-items[$num]
    let $documentURI := if ($cached-item instance of node()) then
        document-uri(root($cached-item))
    else (
    )
    let $serialization-parameters := <output:serialization-parameters>
    ...
    >
        <div class="pos">
            {
                if (string-length($documentURI) > 0) then
                    <a
                        class="badge"
                        data-path="{ $documentURI }"
                        href="{ $documentURI }#{ util:node-id($cached-item) }"
                        title="Click to load source document"
                    >{ $num }</a>
                else
        ...
        <div class="item">
            ...
            <div class="content" style="white-space: pre-wrap">{ $serialized }</div>
        </div>
    </div>
};
...
declare function local:store-in-session ($results as item()*) as element(result) {
    ...
    let $elapsed := if ($startTime) then
        ...
        0
    return <result elapsed="{ $elapsed }" hits="{ count($results) }" />
};
