...
declare function local:get-stack ($session as xs:string?) {
    ...
    return <stack json:array="true" lineno="{ $stack/@lineno }" />
};
    ...
    return <context>
        {
            ...
            return <properties json:array="true">
...
let $response-template := function ($s, $r, $a, $f) {
    <dbgr action="{ $a }" resource="{ $r }" session="{ $s }">{ $f() }</dbgr>
}
return switch ($action)
    case ("init") return
        let $res := request:get-parameter("resource", ())
        let $session := dbgr:init($res)
        return <dbgr action="{ $action }" resource="{ $res }" session="{ $session }">
            { local:get-context($session), local:get-stack($session) }
        </dbgr>
    case ("step") return
        let $session := request:get-parameter("session", ())
        let $res := dbgr:step-over($session)
        return <dbgr action="{ $action }" result="{ $res }" session="{ $session }">
            { local:get-context($session), local:get-stack($session) }
        </dbgr>
    ...
    case ("step-out") return
        ...
        return <dbgr action="{ $action }" res="{ $res }" session="{ $session }">
            {
    ...
    case ("set-breack-point") return
        ()
    default return
        <dbgr>Unknown action { $action }</dbgr>
