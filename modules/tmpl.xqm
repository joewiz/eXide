    ...
    typeswitch ($input)
        case element() return
            element {node-name($input)} {
                for $child in ($input/@*, $input/node())
                return tmpl:expand-template($child, $map)
            }
        case attribute() return
            attribute {node-name($input)} { tmpl:parse($input/string(), $map) }
        case text() return
...
declare function tmpl:parse ($str as xs:string, $parameters as element(parameters)?) {
    ...
    return (: for backwards compatibility: handle recent change to analyze-string :)
    typeswitch ($analyzed)
        case element() return
...
declare function tmpl:process ($node as node(), $parameters as element(parameters)) {
    typeswitch ($node)
        case element(fn:match) return
            ...
            return string($parameters/param[@name = $var]/@value)
        case element() return
            for $child in $node/node()
            return tmpl:process($child, $parameters)
