...
f.test(e[h]))&&b.push({label:e[h],template:"$"+e[h],type:"variable"});for(h=0;h<a.functions.length;h++)"variable"==a.functions[h].type&&(!d||f.test(a.functions[h].name))&&b.push({label:a.functions[h].name,template:"$"+a.functions[h].name,type:"variable"});this.$showPopup(a,c,b,g)};Constr.prototype.functionLookup=function(a,b,d,c){var g=this;$.ajax({url:"modules/docs.xq",dataType:"text",type:"POST",data:{prefix:b},success:function(e){var e=$.parseJSON(e),f=[],h=RegExp("^"+b);$.each(a.functions,function(a,
b){b.name.match(h)&&f.push(b)});e&&(f=f.concat(e));for(var e=[],l=0;l<f.length;l++){var o={label:f[l].signature?f[l].signature:f[l].name,type:f[l].type};f[l].help&&(o.tooltip=f[l].help);e.push(o)}g.getTemplates(a,b,e);g.$showPopup(a,d,e,c)},error:function(a,b){eXide.util.error(b)}})};Constr.prototype.templateLookup=function(a,b,d,c){var g=[];this.getTemplates(a,b,g);this.$showPopup(a,d,g,c)};Constr.prototype.moduleLookup=function(a,b,d,c){var g=this;$.getJSON("modules/find.xq",{prefix:b},function(b){if(b){for(var e=
...
c);h.append("span").text(function(a){return a.name});d.children&&d.children.length&&b.append("ul").selectAll("li").data(d.children).enter().append("li").each(a)};if(d)return d=d.length?d[0]:d,d.isCollection||(d.isResourceOpen=!!l.getDocument(d.key)),h(d);d3.json("modules/collections.xq?root="+(b.datum().key||"/db")+"&view=r",function(a,d){if(!a){var c=b.datum();c.isOpen=!0;c.isLoaded=!0;c.children=d.items.filter(function(a){return".."!=a.name});h(c)}})}}function b(a){eXide.app.$doOpenDocument({name:a.name,
...
function(){this.runCommand({action:"step-out"})};Constr.prototype.runCommand=function(c){var a=this;c.session=this.session;c.resource=this.existURL+this.doc.getPath();$.ajax({type:"POST",url:"modules/debuger.xq",data:c,dataType:"json",success:function(b){$.log("response: %o",b);a.session=b.session;a.editor.gotoLine(b.stack[0].lineno);b.context&&b.context.properties&&$.each(b.context.properties,function(b,c){var d=a.getVariable(c);$(d).appendTo($("div#debuger.content tbody#variables"))});eXide.util.message("Good response. Session: "+
...
e.writable;a.mime=c;a.syntax=eXide.util.mimeTypes.getLangFromMime(c);a.saved=!1;e.line&&this.editor.gotoLine(e.line);this.$initDocument(a)};Constr.prototype.newDocumentFromTemplate=function(a,c){if(!c||0==c.length)this.newDocument(null,a);else{var e=this;$.ajax({url:"modules/get-template.xq",type:"POST",data:{template:c},dataType:"text",success:function(f){for(var j=0,m=0;m<e.documents.length;m++)e.documents[m].path.match(/^__new__(\d+)/)&&(j=parseInt(RegExp.$1));j++;f=new b(f);j=new eXide.edit.Document("new-document "+
...
eXide.edit.Projects=function(c){Constr=function(){this.projects={}};Constr.prototype.findProject=function(a,b){var c=this.getProjectFor(a);c&&null!==c?b(c):this.getProject(a,b)};Constr.prototype.getProject=function(a,b){var e=this;$.getJSON("modules/deployment.xq",{info:a},function(a){if(a){var d=e.projects[a.abbrev];d?c.extend(d,a):(d=a,e.projects[a.abbrev]=d);"function"==typeof b&&b(d)}else"function"==typeof b&&b(null)})};Constr.prototype.getProjectFor=function(a){for(k in this.projects){var b=this.projects[k];
...
modal:!1,autoOpen:!1,width:500,height:360,buttons:{"Sync and Commit":function(){var b=a.gitCommitDialog.find("form"),c=$("[name='git-commit-title']",b).val(),f=$("[name='git-commit-desc']",b).val(),d=c+"\n\n"+f,b=$("[name='start']",b).val();!c||0==c.length?$("#git-commit-status").text("title for commit message is required"):($("#git-commit-status").text("Synchronization in progress ..."),$("#git-commit-status").load("modules/synchronize.xq",{collection:a.currentProject.root,start:b},function(b,c){"success"==
c&&eXide.app.git.command(a.currentProject,"commit",d)}),$(this).dialog("close"))},Cancel:function(){$(this).dialog("close")}}})};eXide.util.oop.inherit(Constr,eXide.events.Sender);Constr.prototype.open=function(c){var a=this,b=null;c&&(b={collection:c});$.ajax({url:"modules/deployment.xq",type:"POST",data:b,success:function(b){a.container.html(b);a.container.form({done:function(){var b=a.container.find("form").serialize();$.ajax({url:"modules/deployment.xq",type:"POST",dataType:"json",data:b,success:function(b){a.container.dialog("close");
...
"title","Open Document");$("#open-dialog").dialog("option","buttons",{cancel:function(){$(this).dialog("close");a.focus()},open:i.openSelectedDocument});$("#open-dialog").dialog("open")},openSelectedDocument:function(a){var b=f.getSelection();b&&i.$doOpenDocument(b);(void 0==a||a)&&$("#open-dialog").dialog("close")},$doOpenDocument:function(b,d,e){b.path=c.normalizePath(b.path);var f=a.getDocument(b.path);if(f&&!e)return a.switchTo(f),d&&d(b),!0;$.ajax({url:"modules/load.xq?path="+b.path,dataType:"text",
...
a.clearErrors();q=n=1;var c=document.getElementById("results-iframe");$(c).show();eXide.app.showResultsPanel();c.contentWindow.document.open("text/html","replace");c.contentWindow.document.write("<html><body><p>Searching ...</p></body></html>");c.contentWindow.document.close();c.src="modules/search.xq?"+b})})},liveReload:function(){var b=a.getActiveDocument();d.findProject(b.getBasePath(),function(a){if(null!=a&&($.log("live reload: %s %s",a.liveReload,a.abbrev),a.liveReload)){var b=a.url.replace(/\/{2,}/,
...
!1)}))}this.breadcrumbs.html(c)};Constr.prototype.setMode=function(a){this.mode=a;"manage"==a?this.grid.setOptions(e):this.grid.setOptions(b)};Constr.prototype.resize=function(){console.log("Resizing canvas...");var a=$(".eXide-browse-main").height();$(".eXide-browse-resources").height(a);this.grid.resizeCanvas();this.grid.focus()};Constr.prototype.update=function(a,b){if(b||a!==this.collection)$.log("Opening resources for %s",a),this.grid.gotoCell(0,0),this.setCollection(a),$('input[name="collection"]').val(a),
this.data.length=0,this.grid.setSelectedRows([]),this.grid.resetActiveCell(),this.grid.setActiveCell(0,1),this.grid.invalidate(),this.grid.onViewportChanged.notify(),this.grid.focus(),this.search=""};Constr.prototype.load=function(a,b){if(!this.loading&&(!this.data[a]||!(b>=this.data.length||this.data[b]))){var c=this;this.loading=!0;b+=20;$.getJSON("modules/collections.xq",{root:this.collection,view:"r",start:a,end:b},function(e){c.loading=!1;for(var h=a;h<=b;h++)c.grid.invalidateRow(h);if(e&&e.items){c.data.length=
e.total;for(h=0;h<e.items.length;h++)c.data[a+h]=e.items[h]}else c.data.length=0;c.grid.updateRowCount();c.grid.render();0==a&&(c.grid.resetActiveCell(),c.grid.setActiveCell(0,1),c.grid.focus())})}};Constr.prototype.hasSelection=function(){var a=this.grid.getSelectionModel().getSelectedRows();return a&&0<a.length};Constr.prototype.getSelected=function(){var a=this.grid.getSelectionModel().getSelectedRows();if(0==a.length)return null;for(var b=[],c=0;c<a.length;c++)b.push(this.collection+"/"+this.data[a[c]].name);
return b};Constr.prototype.createCollection=function(){var a=this;eXide.app.$checkLogin()&&eXide.util.Dialog.input("Create Collection",'<label for="collection">Name: </label><input type="text" name="collection" id="eXide-browse-collection-name"/>',function(){$("#eXide-browse-spinner").show();$.getJSON("modules/collections.xq",{create:$("#eXide-browse-collection-name").val(),collection:a.collection},function(b){$("#eXide-browse-spinner").hide();"fail"==b.status?eXide.util.Dialog.warning("Create Collection Error",
b.message):a.reload()})})};Constr.prototype.deleteCollection=function(){var a=this;eXide.util.Dialog.input("Confirm Deletion","Are you sure you want to delete collection "+a.selected+"?",function(){$("#eXide-browse-spinner").show();$.getJSON("modules/collections.xq",{remove:a.collection},function(b){$("#eXide-browse-spinner").hide();"fail"==b.status?eXide.util.Dialog.warning("Delete Collection Error",b.message):a.reload()})})};Constr.prototype.deleteResource=function(){var a=this.grid.getSelectionModel().getSelectedRows();
if(0!=a.length){for(var b=[],c=0;c<a.length;c++)b.push(this.data[a[c]].key);var e=this;eXide.util.Dialog.input("Confirm Deletion","Are you sure you want to delete the selected resources?",function(){$("#eXide-browse-spinner").show();$.getJSON("modules/collections.xq",{remove:b,root:e.collection},function(a){$("#eXide-browse-spinner").hide();e.reload();"fail"==a.status&&eXide.util.Dialog.warning("Delete Resource Error",a.message)})})}};Constr.prototype.properties=function(){var a=this.grid.getSelectionModel().getSelectedRows();
if(0!=a.length){for(var b=[],c=0;c<a.length;c++)".."!=this.data[a[c]].name&&b.push(this.collection+"/"+this.data[a[c]].name);0<b.length&&($("#resource-properties-content").load("modules/collections.xq",{properties:b}),$("#resource-properties-dialog").dialog("open"))}};Constr.prototype.cut=function(){this.clipboardMode="move";this.copy0()};Constr.prototype.copy=function(){this.clipboardMode="copy";this.copy0()};Constr.prototype.copy0=function(){var a=this.grid.getSelectionModel().getSelectedRows();
this.clipboard=[];for(var b=0;b<a.length;b++){var c=this.data[a[b]].name;"/"!=c.substr(0,1)&&(c=this.collection+"/"+c);this.clipboard.push(c)}$.log("Clipboard: %o",this.clipboard)};Constr.prototype.paste=function(){var a=this;$.log("Pasting resources %o to %s in mode %s",this.clipboard,this.collection,this.clipboardMode);var b={root:this.collection};b[this.clipboardMode]=this.clipboard;$.getJSON("modules/collections.xq",b,function(b){$.log(b.status);"fail"==b.status?eXide.util.Dialog.warning("Delete Resource Error",
b.message):a.reload()})};Constr.prototype.goto=function(a){this.grid.gotoCell(0,0);this.grid.setSelectedRows([]);this.grid.resetActiveCell();this.grid.setActiveCell(a,1);this.grid.invalidate();this.grid.onViewportChanged.notify();this.grid.scrollRowToTop(a);this.grid.focus()};Constr.prototype.focus=function(){this.container.find(".grid-canvas").focus()};Constr.prototype.reload=function(){this.grid.invalidate();this.data.length=0;this.update(this.collection,!0);eXide.app.syncDirectory(this.collection)};
return Constr}();eXide.namespace("eXide.browse.Upload");
...
$(this).dialog("close")},Cancel:function(){$(this).dialog("close")}};"xquery"==a&&(b.Import=function(){eXide.find.Modules.trigger("import");$(this).dialog("close")});$("#select-module-dialog").dialog("option","buttons",b);$("#select-module-dialog").dialog("open")},trigger:function(a){var b=f.getSelectionModel().getSelectedRows();1==b.length&&eXide.find.Modules.$triggerEvent(a,[e[b[0]]])},$init:function(){f.resizeCanvas();f.invalidate();e.length=0;$.getJSON("modules/find.xq",function(a){e.length=a.length;
for(var b=0;b<a.length;b++)e[b]=a[b];f.updateRowCount();f.render();f.setActiveCell(0,0);f.setSelectedRows([0]);$("#module-list").find(".grid-canvas").focus()})},addEventListener:function(a,b,e){(a=c[a])&&a.push({obj:b,callback:e})},$triggerEvent:function(a,b){var e=c[a];if(e)for(var f=0;f<e.length;f++)e[f].callback.apply(e[f].obj,b)}}}();eXide.namespace("eXide.edit.Visitor");eXide.edit.Visitor=function(){Constr=function(){};Constr.prototype.visit=function(c,a){if(c){var b=c.name,e=!1;"function"===typeof this[b]&&(e=!0===this[b](c)?!0:!1);e||this.visitChildren(c,a)}};Constr.prototype.visitChildren=function(c,a){if(c)for(var b=0;b<c.children.length;b++){var e=c.children[b];if(void 0!==a&&"function"===typeof a[e.name])a[e.name](e);else this.visit(e,a)}};return Constr}();eXide.namespace("eXide.edit.XQueryUtils");
