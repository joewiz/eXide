...
eXide.edit.Template = (function () {
  ...
  Constr = function (editor, range, code, type) {
    ...
    if (range) {
      ...
      var cursor = editorUtils.offsetToRowCol(
        this.editor.state,
    ...
    console.log("startLine = %i, endLine = %i", this.startLine, this.endLine);
    this.currentLine = this.startLine;
    this.lineOffset = this.startColumn;
    this.regex = /(?:\$[\w\-:_]+|␣)/g;
    if (this.startColumn > 0) this.regex.lastIndex = this.startColumn;
  };
  ...
  Constr.prototype = {
    ...
    insert: function () {
      if (this.range) {
        var from = editorUtils.rowColToOffset(
          this.editor.state,
          this.range.start.row,
          this.range.start.column
        );
        var to = editorUtils.rowColToOffset(
          this.editor.state,
          this.range.end.row,
      ...
      var lead = editorUtils.offsetToRowCol(
        this.editor.state,
        this.editor.state.selection.main.head
      );
      ...
      if (this.type != "variable") this.nextParam();
      this.editor.focus();
    },
    ...
    nextParam: function () {
      ...
      console.log("lead.row = %i startLine = %i", lead.row, this.startLine);
      // return immediately if the cursor is outside the template
      if (lead.row < this.startLine || lead.row > this.endLine) return false;
      ...
      while (this.currentLine <= this.endLine) {
        ...
        var line =
          lineNum >= 1 && lineNum <= this.editor.state.doc.lines
            ...
            : "";
        console.log("Checking line %s", line);
        ...
        if (this.currentLine > this.endLine && !loop) {
          console.log("loop %i", this.startColumn);
          this.currentLine = this.startLine;
