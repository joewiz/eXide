...
eXide.edit.CompletionSource = (function () {
    ...
    if (state.selection.main.empty) {
      ...
      if (!astNode) {
        ...
        if (parent.name === "VarRef" || parent.name === "VarName") {
          mode = "variables";
          from = editorUtils.rowColToOffset(
            state,
            astNode.pos.sl,
          ...
          astNode =
            parent.name === "VarName" && parent.getParent && parent.getParent.name === "VarRef"
              ? parent.getParent
              : parent;
        } else {
          ...
          if (importStmt) {
            ...
            var line2 =
              lineNum2 >= 1 && lineNum2 <= state.doc.lines ? state.doc.line(lineNum2).text : "";
      ...
      .then(function (results) {
        ...
        if (serverFuncs) {
          ...
          for (var i = 0; i < serverFuncs.length; i++) {
            ...
            var opt = makeFunctionOption(label, snip, null, f.description, null, null);
  ...
  function getVariableCompletions(doc, helper, prefix, from, to, astNode) {
    ...
    for (var j = 0; j < doc.functions.length; j++) {
      if (
        doc.functions[j].type === "variable" &&
        (!prefix || prefixRegex.test(doc.functions[j].name))
      ) {
        options.push({
