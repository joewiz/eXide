...
eXide.edit.XQueryUtils = (function () {
  return {
    ...
    findNodeForRange: function (ast, start, end) {
      var p = ast.pos;
      if (
        eXide.edit.XQueryUtils.inRange(p, start) === true &&
        eXide.edit.XQueryUtils.inRange(p, end) === true
      ) {
        for (var i in ast.children) {
          var child = ast.children[i];
    ...
    inRange: function (p, pos, exclusive) {
      if (p && p.sl <= pos.line && pos.line <= p.el) {
        if (p.sl < pos.line && pos.line < p.el) return true;
        else if (p.sl == pos.line && pos.line < p.el) return p.sc <= pos.col;
        else if (p.sl == pos.line && p.el === pos.line)
          return p.sc <= pos.col && pos.col <= p.ec + (exclusive ? 1 : 0);
        else if (p.sl < pos.line && p.el === pos.line) return pos.col <= p.ec + (exclusive ? 1 : 0);
      }
    },

    samePosition: function (pos1, pos2) {
      return pos1.sl == pos2.sl && pos1.sc == pos2.sc && pos1.el == pos2.el && pos1.ec == pos2.ec;
    },
    ...
    findVariableContext: function (node, varName) {
      ...
      while (parent !== null) {
        if (
          (parent.name === "FLWORExpr" || parent.name === "FunctionDecl") &&
    ...
    findVarDecl: function (node, name) {
      ...
      for (var i = 0; i < refs.length; i++) {
        if (
          (refs[i].getParent && refs[i].getParent.name === "LetBinding") ||
          refs[i].getParent.name === "ForBinding" ||
