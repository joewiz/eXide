...
eXide.edit.XQueryModeHelper = (function () {
  ...
  function addQuickFixActions(annotation) {
    ...
    var prefixMatch =
      /for prefix (\w+)/.exec(annotation.text) || /\[XPST0081\] "(\w+)"/.exec(annotation.text);
    if (prefixMatch) {
        ...
        if (data.errors && data.errors.length > 0) {
          ...
          if (tag && tag.length > 0) {
            ...
            if (tag && tag.length > 0) {
              ...
              $this.editor.dispatch({
                changes: { from: insertPos2, insert: insertText2 },
                selection: { anchor: insertPos2 + insertText2.length },
              });
            }
          }
        }
  ...
  Constr.prototype.parseXQuery = function (doc) {
    ...
    var self = this;
    var versionPref =
      typeof eXide !== "undefined" && eXide.app && eXide.app.getPreference
        ? eXide.app.getPreference("xqueryVersion")
        : "auto";
  ...
  Constr.prototype.quickFix = function (doc, row) {
    if (!row) {
      row = editorUtils.offsetToRowCol(
        this.editor.state,
      ...
      .then(function (refs) {
        if (!Array.isArray(refs) || refs.length === 0) {
          ...
          if (funcName) {
            ...
            var astNode = eXide.edit.XQueryUtils.findNode(doc.ast, {
              line: lead.row,
              col: lead.column,
            });
            var fcall = astNode
              ? eXide.edit.XQueryUtils.findAncestor(astNode, "FunctionCall")
              : null;
            ...
            if (visitor.declaration) {
              refs.unshift({
                line: visitor.declaration.pos.sl,
                column: visitor.declaration.pos.sc,
                name: funcName,
                kind: "function",
              });
            }
        ...
        var items = refs.map(function (ref) {
          return {
            label:
              (ref.kind === "variable" ? ref.name : ref.name) + " \u2014 line " + (ref.line + 1),
            line: ref.line,
            column: ref.column,
          };
        });
        ...
        eXide.util.QuickPicker.show(
          ...
          {
            placeholder: "References (" + items.length + " found)\u2026",
            parentEditor: self.editor,
          }
        );
      })
      .catch(function (err) {
  ...
  Constr.prototype.extractVariable = function (doc) {
    ...
    var range = {
      start: editorUtils.offsetToRowCol(this.editor.state, sel.from),
      end: editorUtils.offsetToRowCol(this.editor.state, sel.to),
    };
    var rangeFrom = editorUtils.rowColToOffset(
      this.editor.state,
      range.start.row,
      range.start.column
    );
    ...
    var contextNode = eXide.edit.XQueryUtils.findAncestor(currentNode, [
      "IntermediateClause",
      "InitialClause",
      "ReturnClause",
    ]);
  ...
  Constr.prototype.extractFunction = function (doc) {
    ...
    var removeFrom = editorUtils.rowColToOffset(
      this.editor.state,
      range.start.row,
      range.start.column
    );
    ...
    var insertPos = this.editor.state.selection.main.head;
    this.editor.dispatch({
      changes: { from: insertPos, insert: params },
      selection: { anchor: insertPos + params.length },
    });
    // reset cursor
  ...
  Constr.prototype.gotoFunction = function (doc, name) {
    ...
    var lines = [];
    for (var i = 1; i <= this.editor.state.doc.lines; i++) {
      lines.push(this.editor.state.doc.line(i).text);
    }
  ...
  Constr.prototype.gotoVarDecl = function (doc, name) {
    ...
    var lines = [];
    for (var li = 1; li <= this.editor.state.doc.lines; li++) {
      lines.push(this.editor.state.doc.line(li).text);
    }
  ...
  Constr.prototype.expandSelection = function (doc) {
    ...
    var selRange = {
      start: editorUtils.offsetToRowCol(this.editor.state, selMain.from),
      end: editorUtils.offsetToRowCol(this.editor.state, selMain.to),
    };
    ...
    if (selRange.start.column == selRange.end.column && selRange.start.row == selRange.end.row) {
      astNode = eXide.edit.XQueryUtils.findNode(doc.ast, {
        line: selRange.start.row,
        col: selRange.start.column,
      });
    } else {
      astNode = eXide.edit.XQueryUtils.findNodeForRange(
        doc.ast,
        { line: selRange.start.row, col: selRange.start.column },
  ...
  Constr.prototype.rename = function (doc) {
    ...
    if (ast) {
      ...
      if (
        dirElem &&
        ast.name == "EQName" &&
        ast.getParent.name == "QName" &&
        ast.getParent.getParent.name == "DirElemConstructor"
      ) {
        ...
        ast.getParent.name == "VarName" ||
        ast.getParent.name == "Param" ||
        ...
        ast.name == "EQName" &&
        (ast.getParent.name == "FunctionDecl" || ast.getParent.name == "FunctionCall")
      ) {
  ...
  Constr.prototype.renderTestResults = function (data) {
    ...
    if (data.failures > 0)
      html += ', <span class="test-failures">' + data.failures + " failures</span>";
    if (data.errors > 0) html += ', <span class="test-errors">' + data.errors + " errors</span>";
    if (data.time) html += " (" + data.time + ")";
    html += "</div>";
    html +=
      '<table class="test-results"><thead><tr><th>Test</th><th>Status</th><th>Details</th></tr></thead><tbody>';
    if (data.testcases) {
  ...
  Constr.prototype.$sortFunctions = function (doc) {
    doc.functions.sort(function (a, b) {
      ...
      else if (b.source && !a.source) return -1;
