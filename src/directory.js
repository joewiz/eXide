...
eXide.edit.Directory = (function () {
  ...
  function fileIcon(d) {
    ...
    var name = d.name || "";
    if (/\.(xml|xq|xquery|xql|xqm|xconf|xhtml|html|htm|svg|xsl|xslt|odd|rng|sch|wsdl)$/i.test(name))
    ...
    var fn = function (d) {
      ...
      var li = sel
        ...
        .attr("class", function (d) {
          return (d.isCollection ? "collection" : "resource") + (d.isResourceOpen ? " open" : "");
        })
      ...
      li.append("i").attr("class", setClass);
      li.append("span").text(function (d) {
        return d.name;
      });

      if (d.children && d.children.length) {
        sel.append("ul").selectAll("li").data(d.children).enter().append("li").each(build);
      }
    };
  ...
  function showContextMenu(d, e, el) {
    ...
    var isRoot = d.key === "/db";
    ctxMenu
      .querySelector('[data-action="new-collection"]')
      .classList.toggle("disabled", !isCollection);
    ctxMenu
      .querySelector('[data-action="new-resource"]')
      .classList.toggle("disabled", !isCollection);
  ...
  function handleAction(action, d, el) {
    switch (action) {
      case "new-collection":
        createCollection(d, el);
        break;
      case "new-resource":
        createResource(d);
        break;
      case "upload":
        uploadToCollection(d, el);
        break;
      case "download":
        downloadItem(d);
        break;
  ...
  function buildPropertiesForm(data, accounts) {
    ...
    (accounts.users || []).sort().forEach(function (u) {
      html +=
        ...
        escapeHtml(u) +
        '"' +
        (u === data.owner ? ' selected="selected"' : "") +
        ">" +
        escapeHtml(u) +
        "</option>";
    });
    ...
    (accounts.groups || []).sort().forEach(function (g) {
      html +=
        ...
        escapeHtml(g) +
        '"' +
        (g === data.group ? ' selected="selected"' : "") +
        ">" +
        escapeHtml(g) +
        "</option>";
    });
  ...
  function showProperties(d) {
    ...
    if (!contentEl) return;
    var propsPromise = fetch(apiPath(d.key)).then(function (r) {
      return r.json();
    });
    var accountsPromise = fetch("api/admin/accounts")
      .then(function (r) {
        return r.ok ? r.json() : { users: [], groups: [] };
      })
      .catch(function () {
        return { users: [], groups: [] };
      });
      ...
      .then(function (data) {
        ...
        if (collection !== "/db") {
          items.unshift({
            name: "..",
            isCollection: true,
            key: collection.substring(0, collection.lastIndexOf("/")) || "/db",
          });
        }
        if (items.length === 0) {
        ...
        items.forEach(function (item) {
          ...
          li.className =
            "dir-flat-item" +
            (item.isCollection ? " dir-flat-collection" : "") +
            (isOpen ? " open" : "");
          var iconClass = item.isCollection ? "fa fa-folder" : "fa " + fileIcon(item);
          li.innerHTML =
            '<i class="' + iconClass + '"></i> <span>' + escapeHtml(item.name) + "</span>";
  ...
  function updateFlatBreadcrumbs(collection) {
    ...
    if (!bc) return;
    bc.innerHTML = "";
    var parts = collection.split("/").filter(function (p) {
      return p.length > 0;
    });
    var path = "";
  ...
  Constr.prototype = {
    toggle: function (state) {
      if (state || state === false) {
        this.__activated = !!state;
      } else {
        this.__activated = !this.__activated;
      }
    ...
    toggleEdit: function (key, state) {
      ...
      sel.attr(
        "class",
        (d.isCollection ? "collection" : "resource") + (d.isResourceOpen ? " open" : "")
      );
    },
  };
