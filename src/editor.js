...
eXide.edit.Editor = (function () {
  ...
  Constr = function (container, menubar, projects) {
    ...
    var outlineData = [
      { label: "collections", cls: "directory" },
      { label: "outline", cls: "outline" },
    ];
  ...
  Constr.prototype.init = function () {
    if (this.documents.length == 0) this.newDocument(null, "xquery");
  ...
  Constr.prototype.newDocument = function (data, type) {
    ...
    var newDocument = new eXide.edit.Document("untitled-" + newDocId, "__new__" + newDocId, text);
  ...
  Constr.prototype.newDocumentFromTemplate = function (mode, template) {
    ...
    fetch("api/templates/" + encodeURIComponent(template))
      ...
      .then(function (data) {
        ...
        newDocId++;
        var newDocument = new eXide.edit.Document(
          "untitled-" + newDocId,
          "__new__" + newDocId,
          data
        );
  ...
  Constr.prototype.openDocument = function (data, mime, resource, externalPath) {
    ...
    else eXide.util.message("Opening " + encodeURI(resource.path));
    ...
    console.log(
      "opening %s, mime: %s, syntax: %s, line: %i",
      resource.name,
      doc.mime,
      doc.syntax,
      resource.line
  ...
  Constr.prototype.$switchView = function (doc) {
    ...
    try {
      ...
      this.editor.dispatch({
        effects: this._readOnlyCompartment.reconfigure(EditorState.readOnly.of(!doc.editable)),
      });
  ...
  Constr.prototype.$setMode = function (doc, setMime) {
    switch (doc.getSyntax()) {
      case "xquery":
        if (setMime) doc.mime = "application/xquery";
        break;
      case "xml":
        if (setMime) doc.mime = "application/xml";
        break;
      case "html":
        if (setMime) {
          if (doc.template == "html") doc.mime = "text/html";
          else doc.mime = "application/xhtml+xml";
        }
        break;
      case "javascript":
        if (setMime) doc.mime = "application/x-javascript";
        break;
      case "css":
        if (setMime) doc.mime = "text/css";
        break;
      case "text":
        if (setMime) doc.mime = "text/text";
        break;
      case "less":
        if (setMime) doc.mime = "application/less";
        break;
      case "tmsnippet":
        if (setMime) doc.mime = "application/tmsnippet";
        break;
      case "json":
        if (setMime) doc.mime = "application/json";
        break;
      case "markdown":
        if (setMime) doc.mime = "text/x-markdown";
    }
  ...
  Constr.prototype.saveDocument = function (resource, successHandler, errorHandler) {
    ...
    if (resource) {
      (($this.activeDoc.path = resource.path), ($this.activeDoc.name = resource.name));
    }

    eXide.util.message("Storing resource " + $this.activeDoc.name + "...");

    var encodedPath = $this.activeDoc.path
      .replace(/^\//, "")
      .split("/")
  ...
  Constr.prototype.getDocument = function (path) {
    ...
    for (var i = 0; i < this.documents.length; i++) {
      if (this.documents[i].path == path) return this.documents[i];
    }
    return null;
  };
  ...
  Constr.prototype.gotoLine = function () {
    ...
    gotoLineEl.querySelectorAll("input[type='text']").forEach(function (inp) {
      inp.value = "";
    });
  ...
  Constr.prototype.addTab = function (doc) {
    ...
    $this.tabs.querySelectorAll("li a").forEach(function (a) {
  ...
  Constr.prototype.saveState = function () {
    ...
    this.documents.forEach(function (doc, index) {
      if (doc.path.match("^__new__.*")) {
        ...
        localStorage["eXide." + i + ".last-line"] = doc.getCurrentLine();
        if (!doc.saved) localStorage["eXide." + i + ".data"] = doc.getText();
      }
      i++;
    });
