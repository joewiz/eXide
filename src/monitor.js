...
eXide.app.Monitor = (function () {
  ...
  function renderSourceCell(sourceKey, sourceType) {
    ...
    if (/^\/db\//.test(src)) {
      return (
        '<td class="mon-source"><a href="#" class="mon-open" data-path="' +
        escapeHtml(src) +
        '" title="' +
        escapeHtml(src) +
        '">' +
    ...
    return (
      '<td class="mon-source" title="' +
      escapeHtml(sourceKey || "") +
      '">' +
      escapeHtml(src) +
      "</td>"
    );
  ...
  Constr.prototype.updateFromWs = function (data) {
    ...
    if (runBody) {
      if (queries.length === 0) {
        ...
        runBody.innerHTML = queries
          .map(function (q) {
            var elapsed = q.elapsed ? (parseInt(q.elapsed) / 1000).toFixed(1) + "s" : "";
            return (
              '<tr class="mon-running">' +
              '<td><button class="mon-kill" data-id="' +
              escapeHtml(q.id) +
              '" title="Kill query">x</button></td>' +
              "<td>" +
              escapeHtml(elapsed) +
              "</td>" +
              '<td class="mon-source">' +
              escapeHtml(q.sourceKey || "") +
              "</td>" +
              "</tr>"
            );
          })
    ...
    if (recentBody) {
      if (recentQueries.length === 0) {
        ...
        recentBody.innerHTML = recentQueries
          .map(function (q) {
    ...
    if (data.db) {
      ...
      if (dbChip)
        dbChip.innerHTML =
          "DB <b>" + (data.db.activeBrokers || "--") + "/" + (data.db.maxBrokers || "--") + "</b>";
    }
  ...
  Constr.prototype.update = function (data) {
    ...
    if (runBody) {
      ...
      var unwrapped = running.map(function (q) {
        return q.value || q;
      });
      var filtered = unwrapped.filter(function (q) {
        return q.caller !== "true";
      });
      if (filtered.length === 0) {
        runBody.innerHTML = '<tr><td colspan="3" class="mon-empty">none</td></tr>';
      } else {
        runBody.innerHTML = filtered
          .map(function (q) {
            var elapsed = q.elapsed ? (parseInt(q.elapsed) / 1000).toFixed(1) + "s" : "";
            return (
              '<tr class="mon-running">' +
    ...
    if (proc && proc.RecentQueryHistory) {
      recent =
        proc.RecentQueryHistory instanceof Array
          ? proc.RecentQueryHistory
          : [proc.RecentQueryHistory];
    }
    var recentBody = document.getElementById("mon-recent-body");
    if (recentBody) {
      // Unwrap row key/value structure from jmx2js
      recent = recent.map(function (q) {
        return q.value || q;
      });
      if (recent.length === 0) {
        recentBody.innerHTML = '<tr><td colspan="2" class="mon-empty">none</td></tr>';
    ...
    if (db) {
      ...
      if (dbChip)
        dbChip.innerHTML =
          "DB <b>" + (db.ActiveBrokers || "?") + "/" + (db.MaxBrokers || "?") + "</b>";
