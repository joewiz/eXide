                ...
                if (gc.name === "Attribute") {
                  ...
                  while (an) {
                    ...
                    if (an.name === "AttributeValue") {
                      ...
                      if (aval.charAt(0) === '"' || aval.charAt(0) === "'")
                        aval = aval.slice(1, -1);
                    }
                    an = an.nextSibling;
                  }
  ...
  Constr.prototype.documentSaved = function (doc) {
    if (/.*\.xconf$/.test(doc.getName())) {
      ...
      eXide.util.Dialog.input(
        "Apply Configuration?",
        "You have saved a collection configuration file. Would you like to " +
          "apply it to collection " +
          collection.replace(/^\/db\/system\/config/, "") +
          " now?",
        function () {
          ...
          var pathParts = basePath
            .replace(/^\/db\/system\/config/, "")
            .split("/")
            .filter(Boolean);
