...
eXide.browse.ResourceBrowser = (function () {
  ...
  Constr.prototype._setupClickHandlers = function () {
    ...
    this.tbody.addEventListener("dblclick", function (e) {
      ...
      if (item.isCollection) {
        var coll;
        if (item.name == "..") coll = self._collection.replace(/\/[^\/]+$/, "");
        else coll = item.key;
  ...
  Constr.prototype._setupContextMenu = function () {
    ...
    this.tbody.addEventListener("contextmenu", function (e) {
      ...
      ctxMenu
        .querySelector('[data-action="paste"]')
        .classList.toggle("disabled", self.clipboard.length === 0);
  ...
  Constr.prototype.update = function (collection, reload) {
    if (!reload && collection === this._collection) return;
  ...
  Constr.prototype.deleteCollection = function () {
    ...
    eXide.util.Dialog.input(
      "Confirm Deletion",
      "Are you sure you want to delete collection " + self.selected + "?",
      function () {
        var spinner = document.getElementById("eXide-browse-spinner");
        spinner.style.display = "";
  ...
  Constr.prototype.deleteResource = function (row) {
    ...
    eXide.util.Dialog.input(
      ...
      function () {
        ...
        spinner.style.display = "";
        var promises = resources.map(function (r) {
          return fetch(apiPath(r), { method: "DELETE" }).then(function (resp) {
            return resp.json();
          });
        });
        Promise.all(promises).then(function (results) {
          spinner.style.display = "none";
          self.reload();
          var err = results.find(function (d) {
            return d.error;
          });
          if (err) {
  ...
  Constr.prototype.properties = function () {
    ...
    if (resources.length > 0) {
      ...
      var propsPromise = fetch(apiPath(resources[0])).then(function (r) {
        return r.json();
      });
  ...
  Constr.prototype.paste = function () {
    var self = this;
    console.log(
      "Pasting resources %o to %s in mode %s",
      this.clipboard,
      this._collection,
      this.clipboardMode
    );
    ...
    fileInput.addEventListener("change", function () {
      ...
      Promise.all(promises).then(function () {
        ...
        if (deployInput.checked) {
          var xarFiles = files.filter(function (f) {
            return /\.xar$/i.test(f.name);
          });
  ...
  Constr = function () {
    ...
    if (fileUploadInput) {
      uploadFiles(
        ...
        progressAll || document.createElement("div"),
        spinner
      );
  ...
  Constr = function (container) {
    ...
    button.addEventListener("click", function (ev) {
      ...
      if (rows.length === 1 && rows[0].isCollection) {
        var coll =
          rows[0].name === ".." ? self.resources._collection.replace(/\/[^\/]+$/, "") : rows[0].key;
