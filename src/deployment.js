...
eXide.edit.Projects = (function () {
  ...
  Constr.prototype.restoreState = function () {
    ...
    if (localStorage["eXide.projects"]) {
      ...
      if (typeof this.projects != "object") this.projects = {};
    }
...
eXide.edit.PackageEditor = (function () {
  Constr = function (projects) {
    ...
    runDialogEl.querySelector("input[name='live-reload']").addEventListener("click", function (ev) {
      ...
      document
        ...
        .setAttribute(
          "class",
          $this.currentProject.liveReload ? "fa fa-check-square-o" : "fa fa-square-o"
        );
    });
    ...
    this.syncDialog = eXide.util.DialogManager.create(syncDialogEl, {
      ...
      buttons: {
        ...
        Synchronize: function () {
          var dir = syncDialogEl.querySelector('input[name="dir"]').value;
          if (!dir || dir.length == 0) {
            document.getElementById("synchronize-report").textContent =
          ...
          var params = new URLSearchParams({
            ...
            "expand-xincludes": document.getElementById("expand-xincludes-on-download-package")
              .checked,
            "omit-xml-declarationaration": document.getElementById(
              "omit-xml-declaration-on-download-package"
            ).checked,
          });
  ...
  Constr.prototype.synchronize = function (collection) {
    ...
    $this.projects.findProject(collection, function (project) {
      if (!project) {
        eXide.util.error(
          "Application not found: The document currently opened in the editor " +
      ...
      if (!eXide.app.login.isAdmin) {
        eXide.util.error(
          "You need to be logged in as an admin user with dba role " + "to use this feature."
        );
        return;
      }
