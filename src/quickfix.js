...
eXide.edit.XQueryQuickFix = (function () {
      ...
      getResolutions: function (helper, editor, doc, annotation, ast) {
        ...
        if (matches && matches.length === 2) {
          if (ast && ast.getParent.name === "FunctionCall") {
            ...
            ast &&
            (ast.name === "EQName" ||
              ast.getParent.name === "ElementTest" ||
        ...
        if (ast.getParent.name === "VarName") {
          return [
            ...
            },
            {
              resolve: function (helper, editor, doc, annotation) {
                var template;
                var contextNode = eXide.edit.XQueryUtils.findAncestor(ast, [
                  "IntermediateClause",
                  "InitialClause",
                  "ReturnClause",
                ]);
                if (contextNode) {
                  ...
                  contextNode = eXide.edit.XQueryUtils.findAncestor(
                    ast,
  ...
  function unusedNamespaceFix(helper, editor, doc, annotation) {
    var nsNode = eXide.edit.XQueryUtils.findNode(doc.ast, {
      line: annotation.row,
      col: annotation.column + 1,
    });
    ...
    var lastLine =
      lineNum >= 1 && lineNum <= view.state.doc.lines ? view.state.doc.line(lineNum).text : "";
    var from, to;
  ...
  function getResolutions(helper, editor, doc, annotation) {
    ...
    for (var i = 0; i < quickFixes.length; i++) {
      if (quickFixes[i].regex.test(annotation.text)) {
        var ast = eXide.edit.XQueryUtils.findNode(doc.ast, {
          line: annotation.row,
          col: annotation.column + 1,
        });
