...
var staticAnalysis = (function () {
  ...
  function analyze(ast) {
    ...
    var declaredNS = {
      jn: {
        ns: "http://www.jsoniq.org/functions",
    ...
    function makeScope(pos, parent) {
      return {
        pos: { sl: pos.sl, sc: pos.sc, el: pos.el, ec: pos.ec },
        parent: parent,
        children: [],
        varDecls: {},
        varRefs: {},
    ...
    visitor.visitExprSingles = function (node) {
      for (var i = 0; i < node.children.length; i++) {
        var child = node.children[i];
        if (
          child.name === "ExprSingle" ||
          child.name === "VarValue" ||
          child.name === "VarDefaultValue"
        ) {
          visitor.visit(child);
        }
      }
    ...
    visitor.AnnotatedDecl = function (node) {
      ...
      var Handler = function () {
        this.FunctionDecl = function (fnode) {
          isExternal =
            fnode.children[fnode.children.length - 1].value &&
          ...
          fnode.arity = "" + fnArity;
          if (declaredFunctions[eQName] && declaredFunctions[eQName][fnArity] !== undefined) {
