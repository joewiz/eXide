      ...
      cy.get(".panel-west")
        ...
        .then((initialWidth) => {
          ...
          cy.get(".panel-west .resize-handle").then(($handle) => {
            ...
            cy.get(".panel-west .resize-handle").trigger("mousedown", {
              pageX: startX,
              pageY: startY,
              which: 1,
            });
            cy.get(".layout").trigger("mousemove", { pageX: startX + 80, pageY: startY, which: 1 });
            cy.document().trigger("mouseup");

            cy.get(".panel-west")
              .invoke("outerWidth")
              .should("be.gt", initialWidth + 40);
          });
        });
    });
      ...
      cy.get(".panel-south")
        ...
        .then((initialHeight) => {
          ...
          cy.get(".panel-south .resize-handle").then(($handle) => {
            ...
            cy.get(".panel-south .resize-handle").trigger("mousedown", {
              pageX: startX,
              pageY: startY,
              which: 1,
            });
            cy.get(".layout").trigger("mousemove", {
              pageX: startX,
              pageY: startY - 100,
              which: 1,
            });
