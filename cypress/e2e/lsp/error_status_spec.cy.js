describe("Error status UI", () => {
  ...
  it("shows pill and panel when an error is written to #error-status", () => {
    ...
    cy.get("#exide-err-pill").should("have.class", "has-error").and("be.visible");

    cy.get("#exide-err-pill-label").invoke("text").should("have.length.greaterThan", 0);

    cy.get("#exide-err-panel").should("have.class", "ep-open").and("be.visible");

    cy.get("#exide-err-panel-body").should("contain", "XPST0017");
  });
