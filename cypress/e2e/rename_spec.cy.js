describe("Rename (multi-cursor)", () => {
  ...
  it("creates multi-cursor selection for variable rename", () => {
    ...
    cy.get(".eXide-toast", { timeout: 5000 }).should("contain", "Editing 3 occurrence");
  });
  ...
  it("renames variable from $ sign position", () => {
    ...
    cy.get(".eXide-toast", { timeout: 5000 }).should("contain", "Editing 2 occurrence");
  });
  ...
  it("shows message when cursor is not on a renameable symbol", () => {
    ...
    cy.get(".eXide-toast", { timeout: 5000 }).should("contain", "cursor");
  });
});
