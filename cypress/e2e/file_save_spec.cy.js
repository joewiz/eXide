describe("File save", () => {
  ...
  function waitForSaveDialog() {
    cy.get("#open-dialog").closest(".eXide-dialog", { timeout: 5000 }).should("have.attr", "open");
  }

  function saveDialogShouldBeClosed() {
    cy.get("#open-dialog").closest(".eXide-dialog").should("not.have.attr", "open");
  }

  function clickDialogButton(label) {
    cy.get("#open-dialog")
      .closest(".eXide-dialog")
      .find(".eXide-dialog-buttons button")
      .contains(label)
      .click();
  }
  ...
  it("warns when saving XQuery with non-XQuery extension", () => {
    ...
    cy.get(".eXide-dialog[open]", { timeout: 5000 }).should("contain", "non-XQuery file extension");
  });
