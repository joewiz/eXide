describe("Monitor panel", () => {
  ...
  it("token endpoint returns server status for admin user", () => {
    cy.window()
      .then((win) => {
        return win.fetch("api/admin/status").then((r) => r.json());
      })
      .then((data) => {
  ...
  it("starts polling when toggled open by admin", () => {
    ...
    cy.get("#mon-mem-nums", { timeout: 15000 })
      .invoke("text")
      .should("match", /\d+ \/ \d+ MB/);
  });
  ...
  it("shows running query and allows killing it", () => {
    ...
    cy.get("#mon-running-body .mon-running", { timeout: 5000 }).should("have.length.at.least", 1);
    ...
    cy.get("#mon-running-body .mon-empty", { timeout: 15000 }).should("exist");
  });
