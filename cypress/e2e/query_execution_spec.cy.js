describe("Query execution", () => {
  ...
  it("runs a simple XQuery and shows results", () => {
    ...
    cy.get(".panel-south .results", { timeout: 10000 }).should("not.be.empty");
  ...
  it("shows an error for invalid XQuery", () => {
    ...
    cy.get("#error-status", { timeout: 10000 }).invoke("text").should("have.length.greaterThan", 0);
  });
  ...
  it("displays correct result count", () => {
    ...
    cy.get(".panel-south .current", { timeout: 10000 }).invoke("text").should("contain", "5");
  });
  ...
  it("shows a toast notification with result count", () => {
    ...
    cy.get(".eXide-toast", { timeout: 10000 }).should("contain", "returned").and("contain", "item");
  });
  ...
  it("paginates results with next/previous buttons", () => {
    ...
    cy.get(".panel-south .results .content").should("have.length", 10);

    // Click next page
    cy.get(".panel-south .next").click();
    cy.get(".panel-south .current").invoke("text").should("contain", "11 to 20 of 25");

    // Click next again — partial page
    cy.get(".panel-south .next").click();
    cy.get(".panel-south .current").invoke("text").should("contain", "21 to 25 of 25");
    ...
    cy.get(".panel-south .current").invoke("text").should("contain", "1 to 10 of 25");
